...
public class ApplicationFeatureRbac {
  ...
  public void authorizeUserForFeatureVisibility(String userName) {
    ...
    if (statusCode == HttpStatus.NOT_FOUND.value() || statusCode == HttpStatus.OK.value()) {
      ...
      log.warn("Access forbidden for user {} on feature type {}", userName, RbacFeatureType.APP.description);
  ...
  public void authorizeUserForApplicationId(
    ...
    switch (method) {
      case GET:
        ResponseEntity<PermissionModel> response = oesAuthorizationService
          .fetchPermissions(username, RbacFeatureType.APP.name(), applicationId, username);
        int statusCode = response.getStatusCodeValue();
        permission = response.getBody();

        if ((statusCode == HttpStatus.NOT_FOUND.value() || statusCode == HttpStatus.OK.value())
          && permissionEvaluator.hasPermission(username, applicationId, RbacFeatureType.APP.name(), Authorization.EXECUTE)) {
          log.info("Authorized user for Application ID with status code :{}, {}", applicationId, statusCode);
          log.info("permissions for the GET API : {}", permission);
        } else if (permission == null
          || !permission.getPermissions().contains(PermissionEnum.view.name()) || statusCode == HttpStatus.FORBIDDEN.value()) {
          throw new AccessForbiddenException(
            YOU_DO_NOT_HAVE
  ...
  public void authorizeUserForPipelineId(String username, String endpointUrl, String httpMethod) {
    ...
    switch (method) {
      case GET:
        ResponseEntity<Map<String, String>> response = oesAuthorizationService.isAuthorizedUser(
                        username,
                        PermissionEnum.view.name(),
                        null,
                        pipelineId,
  ...
  public void authorizeUserForGateId(String username, String endpointUrl, String httpMethod) {
    ...
    switch (method) {
      case GET:
              ResponseEntity<Map<String, String>> response = oesAuthorizationService
                    .isAuthorizedUser(
                        username,
                        PermissionEnum.view.name(),
                        null,
      ...
      case GET:
              ResponseEntity<Map<String, String>> response = oesAuthorizationService
                    .isAuthorizedUser(
                        ...
                        username);
        isAuthorized =
          Boolean.parseBoolean(response.getBody().get("isEnabled"));
        int statusCode = response.getStatusCodeValue();
  ...
  public void authorizeUserForPolicyGateTrigger(HttpServletRequest request, Object input) {
    ...
    } else if (inputJson.has("name")) {
      appName = inputJson.get("name").getAsString();
    }
  ...
  public void authorizeUserForVerificationAndTestVerificationGateTrigger(
    ...
    ResponseEntity<Map<String, String>> response = oesAuthorizationService.isAuthorizedUser(
                    ...
                    appName,
                    username);
    isAuthorized = Boolean.parseBoolean(response
                .getBody()
                .get("isEnabled"));
    int statusCode = response.getStatusCodeValue();
  ...
  private static void populateCustomGateTriggerEndpoints() {

    runtime_access.add("/visibilityservice/v5/approvalGates/{id}/trigger");
    runtime_access.add("/visibilityservice/v4/approvalGates/{id}/trigger");
    runtime_access.add("/visibilityservice/v3/approvalGates/{id}/trigger");
    runtime_access.add("/visibilityservice/v2/approvalGates/{id}/trigger");
    runtime_access.add("/visibilityservice/v1/approvalGates/{id}/trigger");
  }
}
