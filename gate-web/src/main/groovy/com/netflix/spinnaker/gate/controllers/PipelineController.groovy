...
class PipelineController {
  ...
  HttpEntity invokePipelineConfigViaEcho(@PathVariable("application") String application,
                                         @PathVariable("pipelineNameOrId") String pipelineNameOrId,
                                         @RequestBody(required = false) Map trigger) {
    log.info("trigger pipeline (application: {}, pipelineId: {})",value("application", application), value("pipelineId", pipelineNameOrId))
    trigger = trigger ?: [:]
    AuthenticatedRequest.setApplication(application)
    try {
  ...
  Map evaluateVariables(@Parameter(name = "Execution id to run against", required = true)
                        @RequestParam("executionId") String executionId,
                        @Parameter(name = "Comma separated list of requisite stage IDs for the evaluation stage", required = false)
                        @RequestParam(value = "requisiteStageRefIds", defaultValue = "") String requisiteStageRefIds,
                        @Parameter(name = "Version of SpEL evaluation logic to use (v3 or v4)", required = false)
                        @RequestParam(value = "spelVersion", defaultValue = "") String spelVersionOverride,
                        @Parameter(name = "List of variables/expressions to evaluate",
                          required = true)
